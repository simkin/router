...
xml=$(curl --silent --header "Content-Type: text/xml;charset=UTF-8" --header "SOAPAction:urn:syncer#ListDnsRecords" --data @list.xml http://api.syncer.com/index.php)
SUBDOMAINS=$(printf '%s\n' "$xml" | xmlstarlet sel -t -m "//item[type='A']" -v "name" -o " ")
array=( $SUBDOMAINS )
OLDIP=$(printf '%s\n' "$xml" | xmlstarlet sel -t -m "//item[1][type='A']" -v "data")
...
echo Calling: xmlstarlet sel -t -m "//item[name='$i']" -v "data"
IP=$(printf '%s\n' "$xml" | xmlstarlet sel -t -m "//item[name='$i']" -v "data")
